...
from yumeko.games.word_chain.strings import (
    lobby_text,
    join_countdown_text,
    joined_text,
    not_enough_text,
...
async def wc_cmd(client, message: Message):
    if message.chat.type == ChatType.PRIVATE:
        await message.reply_text("Word Chain can only be played in groups.")
        return
    ...
    if not await is_admin(client, chat_id, user.id):
        await message.reply_text("Only group admins can start Word Chain.")
        return
    ...
    lock_game(chat_id, "Word Chain")

    game = get_game(chat_id)
    join_game(chat_id, user)

    game_started("Word Chain", chat_id)
...
async def join_cmd(client, message: Message):
    ...
    if not ok:
        if reason == "already_joined":
            await message.reply_text(
                "You're already in, darling~",
                reply_to_message_id=message.id,
            )
        elif reason == "full":
            await message.reply_text(
                "The table is full, darling~",
                reply_to_message_id=message.id,
            )
        return
...
async def wc_join_callback(client, query: CallbackQuery):
    ...
    if not game or game["status"] != "joining":
        await query.answer("No open Word Chain lobby, darling~", show_alert=True)
        return
    ...
    await query.answer("You joined Word Chain!")
...
async def send_turn(client, chat_id: int):
    ...
    asyncio.create_task(turn_timeout(client, chat_id, token, seconds))


async def turn_timeout(client, chat_id: int, token: int, seconds: int):
    ...
    if winner:
        ...
        game_finished("Word Chain", winner["name"])
        end_game(chat_id)
        unlock_game(chat_id)
        return
...
async def wc_word_checker(client, message: Message):
    chat_id = message.chat.id
    user = message.from_user
    ...
    if not current_player or current_player["id"] != user.id:
        return
    ...
    if not is_valid_dictionary_word(word):
        await message.reply_text(
            "❌ Wrong word, darling~\n\nThis word does not exist in Yumeko's dictionary.",
            reply_to_message_id=message.id,
        )
        return
    ...
    await reward_valid_word(user.id, result)
...
def register_word_chain_handlers(app):
    app.add_handler(
        MessageHandler(wc_cmd, filters.command(["wordchain", "wc"])),
        group=30,
    )
    ...
    app.add_handler(
        CallbackQueryHandler(wc_join_callback, filters.regex("^wc_join$")),
        group=30,
    )

    app.add_handler(
        MessageHandler(wc_word_checker, filters.text & filters.group),
        group=-40,
    )